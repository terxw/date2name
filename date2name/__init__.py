#!/usr/bin/env python3
# -*- coding: utf-8 -*-
PROG_VERSION = "Time-stamp: <2024-04-26>"
...
REGEX_PATTERNS = {
    ...
    "LONG": re.compile(r"^(\d{14})(\d{3})"),
    "LONG_C": re.compile(r"^(\d{8})_(\d{6})(\d{3})"),
    "COMPACT": re.compile(r"^(\d{4})([01]\d)([0123]\d)([- _])"),
    "STANDARD": re.compile(r"^(\d{4})-([01]\d)-([0123]\d)([- _])"),
    "MONTH": re.compile(r"^(\d{4})-([01]\d)(?!-[0123]\d)([- _])"),
    "WITHTIME_AND_SECONDS": re.compile(
        r"^(\d{4})-([01]\d)-([0123]\d)([T :_-])([012]\d)([:.-])([012345]\d)([:.-])([012345]\d)([- _.])"
    ),
    "WITHTIME_NO_SECONDS": re.compile(
        r"^(\d{4})-([01]\d)-([0123]\d)([T :_-])([012]\d)([:.-])([012345]\d)([- _.])"
    ),
    "WITHSECONDS": re.compile(
        r"^(\d{4})-([01]\d)-([0123]\d)([T :_-])([012]\d)([:.-])([012345]\d)([:.-])([012345]\d)([- _])"
    ),
}
...
parser.add_option(
    "--version", dest="version", action="store_true", help="display version and exit"
)
...
def get_converted_basename(matched_pattern, item):
    ...
    if matched_pattern == "compact":
        ...
        item_month = item[5:7]
        logging.info(
            '%s no datestamp information for day, so I will take "00" for conversion'
            % item
        )
        item_day = "00"
    ...
    if options.compact:
        logging.debug("%s: Conversion to format" % item)
        return item_year + item_month + item_day + name_without_datestamp
    elif options.month:
        logging.debug("%s: Conversion to month format" % item)
        ...
        logging.warn(
            "%s: Sorry! Conversion to withtime-format not implemented yet, taking standard format"
            % item
        )
        return item_year + "-" + item_month + "-" + item_day + name_without_datestamp
    else:
...
def get_timestamp_from_file(formatstring, item):
    ...
    if options.ctime and platform.system() == "Darwin":
        ...
        return (
            time.strftime(formatstring, time.localtime(os.stat(item).st_birthtime))
            + delimiter_char
            + item
        )
    elif options.ctime and platform.system() != "Darwin":
        return (
            time.strftime(formatstring, time.localtime(os.path.getctime(item)))
            + delimiter_char
            + item
        )
    elif options.mtime:
        return (
            time.strftime(formatstring, time.localtime(os.path.getmtime(item)))
            + delimiter_char
            + item
        )
    else:
        logging.error(
            "internal error: not ctime nor mtime chosen! You can correct this by giving a parameter"
        )
...
def generate_new_basename(formatstring, basename):
    ...
    withtime_and_seconds_components = REGEX_PATTERNS["WITHTIME_AND_SECONDS"].match(
        basename
    )
    withtime_no_seconds_components = REGEX_PATTERNS["WITHTIME_NO_SECONDS"].match(
        basename
    )
    ...
    if options.nocorrections or REGEX_PATTERNS["NODATESTAMP"].match(basename):
        logging.debug(
            'basename "'
            + basename
            + '" matches nodatestamp-pattern or option nocorrections '
        ...
        if options.withtime:
            if not (
                withtime_and_seconds_components.group(1) == "T"
        ...
        if options.withtime:
            if not (
                withtime_no_seconds_components.group(1) == "T"
                and withtime_and_seconds_components.group(3) == "."
            ):
                logging.debug(
        ...
        logging.debug(
            'basename "%s" does not match any known datestamp-pattern' % basename
        )
        new_basename = get_timestamp_from_file(formatstring, basename)
...
def remove_timestamp_from_basename(basename, list_of_regular_expressions=None):
    ...
    if not list_of_regular_expressions or not isinstance(
        list_of_regular_expressions, list
    ):
...
def handle_item(path, basename, formatstring):
    ...
    if options.remove:
        ...
        logging.debug(
            '••••••••••••••••········· adding timestamp to base "%s"' % basename
        )

        if options.onlyfiles and os.path.isdir(basename):
            logging.debug(
                'skipping directory "%s" because of command line option "-f"' % basename
            )
            return

        if options.onlydirectories and os.path.isfile(basename):
            logging.debug(
                'skipping file "%s" because of command line option "-d"' % basename
            )
            return
    ...
    if options.dryrun:
        ...
        if basename == new_basename:
            ...
            if len(new_basename) > MAX_PATHLENGTH:
                logging.error(
                    "ERROR: the current file needs to be renamed with a file name length of "
                    + "%i which is greater than %i. "
                    % (len(new_basename), MAX_PATHLENGTH)
                    + 'This usually causes "[Errno 36] File name too long" and therefore '
                    + "I ignore this file for now. Please shorten file name for at "
                    + "least %i characters and try again."
                    % (len(new_basename) - MAX_PATHLENGTH)
                )
                sys.exit(1)
            else:
...
def main():
    ...
    if options.onlyfiles and options.onlydirectories:
        parser.error(
            "please use either option files (-f) or option directories (-f) or none of them (for renaming directories and files)"
        )
    ...
    if (
        (options.compact and options.withtime)
